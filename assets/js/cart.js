...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.body.addEventListener('click', async (e) => {
    //  Nếu không phải nút thêm vào giỏ hoặc mua ngay thì thoát
    ...
    try {
      ...
      if (data.success) {
        ...
        showErrorMessage(data.error || 'Có lỗi xảy ra khi thêm vào giỏ hàng.')
      }
    } catch (err) {
  ...
  cartContainer.innerHTML = data.cart
    ...
    <div class="items-center hidden p-6 space-x-4 lg:flex" data-id="${item.id}">
      ...
      <div class="flex items-center space-x-3">
        ...
        <span class="text-lg font-medium text-gray-900 min-w-[2rem] text-center cart-qty" data-id="${
          item.id
        }">${item.qty}</span>
...
function showErrorMessage(message) {
  // 🔹 Nếu đã có toast, chỉ cập nhật nội dung & reset timer
  ...
  if (toast) {
    ...
    // Dùng style inline để tránh lỗi transform context
    ...
    document.documentElement.appendChild(toast) // gắn trực tiếp vào <html>
  }

  // 🔹 Reset hiệu ứng fade nếu đang có
  toast.classList.remove('opacity-0')

  // 🔹 Thiết lập (hoặc reset) timer ẩn
